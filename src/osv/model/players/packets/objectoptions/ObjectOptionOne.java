...
import osv.clip.doors.DoorHandler;
import osv.model.content.CrystalChest;
import osv.model.content.SkillChest;
import osv.model.content.Obelisks;
...
import osv.model.minigames.lighthouse.DisposeType;
import osv.model.minigames.pest_control.PestControl;
import osv.model.minigames.rfd.DisposeTypes;
...
import osv.model.multiplayer_session.duel.DuelSessionRules.Rule;
...
import osv.model.players.skills.FlaxPicking;
...
import osv.model.players.skills.woodcutting.Tree;
import osv.model.players.skills.woodcutting.Woodcutting;
...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		int[] bludgeonPieces = { 13274, 13275, 13276 };
		switch (objectType) {
		
		case 29747:
			//Server.getGlobalObjects().replace(, add);
			break;
		...
		case 27057:
			if (!c.getItems().playerHasAllItems(bludgeonPieces)) {
				...
				c.getItems().deleteItem(13274, 1);
				c.getItems().deleteItem(13275, 1);
				c.getItems().deleteItem(13276, 1);
		...
		case 20877:
			AgilityHandler.delayFade(c, "CRAWL", 2714, 9564, 0, "You crawl into the entrance.", "and you end up in a dungeon.", 3);
		...
		case 11835:
			...
			break;
		case 11836:
			AgilityHandler.delayFade(c, "CRAWL", 2862, 9572, 0, "You crawl into the entrance.", "and you end up in karamja volcano dungeon.", 3);
			break;
		...
		case 3829:
			c.getPA().movePlayer(3226, 3109, 0);
			break;
		...
		case 26761:
			...
			break;
		case 7813:
			c.getFarming().fillCompostBucket();
			break;
		case 2492:
		case 15638:
		case 7479:
		...
		case 17387:
			if (c.getRights().isOrInherits(Right.CONTRIBUTOR)) {
				AgilityHandler.delayEmote(c, "CLIMB_UP", 2858, 2954, 0, 2);
			}
			break;
		case 25824:
			c.turnPlayerTo(obX, obY);
...
//		case 7407:
		...
		case 412:
			...
			if (c.absY >= 3504 && c.absY <= 3507) {
				if (c.specAmount < 10.0) {
					if (c.specRestore > 0) {
						c.sendMessage("You have to wait another " + c.specRestore + " seconds to use this altar.");
						return;
					}
					if (c.getRights().isOrInherits(Right.CONTRIBUTOR)) {
						c.specRestore = 120;
		...
		case 26363:
			...
			if (c.gwdAltar > 0) {
				c.sendMessage("You have to wait another " + c.gwdAltar + " seconds to use this altar.");
				return;
			}
			if (c.playerLevel[5] < c.getPA().getLevelForXP(c.playerXP[5])) {
				c.startAnimation(645);
