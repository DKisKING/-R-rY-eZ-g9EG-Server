...
import osv.model.content.IronManInterface.Action;
import osv.model.holiday.HolidayController;
import osv.model.holiday.christmas.ChristmasToy;
import osv.model.holiday.christmas.ChristmasToyUpgrade;
...
import osv.model.players.skills.slayer.Task;
import osv.server.data.SerializablePair;
...
public class DialogueHandler {
	...
	public static int[] UNSIRED_REWARDS = { 7979, 13265, 4151, 13274, 13275, 13276, 13277 };
	...
	public void sendDialogues(int dialogue, int npcId) {
		...
		case 851:
			sendOption5("Teleport me to the raids dungeon", "I want to look into your store", "What is this?", "Is this safe?", "You cannot help me right now..");
		...
		case 852:
			...
			//Is this safe?
		case 853:
			sendNpcChat("NO!", "Raids dungeons are not safe, be aware and join only", "if you are willing to risk what you bring!");
			c.nextChat = 851;
			break;
			//Teleport me to
		case 854:
		...
		case 812:
			...
			break;
		case 813:
			sendOption2("Yes I am aware, and I want to leave", "No, let me store my valueables in my safe-box");
			c.dialogueAction = 813;
			break;
			//Safe box
		case 818:
			sendNpcChat("Using quick-sets will replace your inventory and worn items", "You have been warned!!", "@red@Deposit your valueable items/blood money.");
			c.nextChat = 819;
			break;
		case 819:
		...
		case 706:
			if (c.getItems().playerHasItem(11286) && c.getItems().playerHasItem(1540) && c.getItems().playerHasItem(995, 500_000) && c.votePoints >= 5) {
				...
				sendNpcChat("You do not pocess the items I required! Go away!");
			}
			c.nextChat = -1;
			break;
		...
		case 61:
			sendOption5("Green", "White", "", "", "Previous"); //Green
			c.dialogueAction = 56;
			break;
...
//			}
		...
		case 72:
			sendOption2("Sacrifice your firecape for a chance at TzRek-Jad?", "Yes, I know I won't get my cape back..", "No, I like my cape!.");
			c.dialogueAction = 72;
			break;
		...
		case 101:
			sendPlayerChat1("How about 22.770.000 gold?.");
			c.nextChat = 102;
			break;
		...
		case 1:
			sendNpcChat("Come back when you have finished all your achievements.");
			c.nextChat = -1;
			break;
		case 2:
			sendNpcChat("This cape costs 99.000gp.");
			c.nextChat = -1;
			break;
		case 4:
			...
			break;
		case 7:
			sendNpcChat("There you go saddo, goodjob.");
			c.nextChat = -1;
			break;
...
//			break;
		...
		case 3311:
			sendPlayerChat1("Great, thank you grumpy man!");
			c.nextChat = 0;
			break;
		...
		case 14400:
			...
			break;
		case 14401:
			sendOption5("Gnome Course", "Barbarian Course", "Wilderness Course @red@(DEEP WILD)", "", "Rooftop Agility");
		...
		case 14011:
			...
			break;
		case 14012:
			sendPlayerChat3("Yeah! I've never seen one before.", "I guess it's one small step for NPC's,", "one giant leap for Tyras!");
			c.nextChat = 14013;
			break;
		case 14013:
		...
		case 5:
			...
			break;
		case 6:
			sendNpcChat4("The way the game works is as follows...", "You will be teleported to the wilderness,", "You must kill mages to recieve points,",
		...
		case 23000:
			c.getDH().sendStartInfo("As you collect your reward, you notice an aweful smell.", "You look below the remaining debris to the bottom of the",
		...
		case 525:
			...
			sendNpcChat4("Although they are not tough, they move quickly.", "They move from location to location every 10 minutes.",
					"You will need to be quick, I have somethign to help.", "Here, take this locating crystal to find them.", c.talkingNpc, "Grim Reaper");
			c.nextChat = 526;
			break;
		...
		case 529:
			...
			if (total < 50) {
				if (total == 0) {
					...
					sendNpcChat3("You have found " + total + ", thats a good start.", "However, you need to search harder.", "Hurry, there isn't much time.", c.talkingNpc,
							"Grim Reaper");
				} else {
		...
		case 584:
			sendNpcChat("Ho Ho Ho, aren't you a little rascal.", "I hope you like smithing because you will", "be gettings tons of coal this year.");
			c.nextChat = -1;
			break;
		...
		case 612:
			sendOption2("Use christmas cracker, I may get a partyhat.", "No, I want to keep my parthay.");
		...
		case 614:
			...
			try {
				...
				if (amount > 500) {
					message = "The gods are astounded by your generousity.";
				}
		...
		case 616:
			...
			top5.forEach(entry -> sb.append(StringUtils.capitalize(entry.getKey()) + " contributed @blu@" + Misc.insertCommas(Integer.toString(entry.getValue())) + "-"));
		...
		case 617:
			...
			for (Entry<String, Integer> entry : winners) {
				sb.append((spot++) + ".) " + StringUtils.capitalize(entry.getKey()) + " (@blu@" + Misc.insertCommas(Integer.toString(entry.getValue())) + "@bla@)-");
			}
			sendStatement(sb.toString().split("-"));
			c.nextChat = 613;
			break;
		...
		case 622:
			sendNpcChat("If an item degrades and dissapears, you can claim it.", "It costs 2x as much to claim it then it does to repair.");
			c.nextChat = 620;
			break;
		...
		case 623:
			...
			String item1 = ItemAssistant.getItemName(degradeList[0].getItemId()) + " @red@" + (DegradableItem.forId(degradeList[0].getItemId()).get().getCost() * 2) + " PKP";
			String item2 = "";
			String item3 = "";
			String item4 = "";
		...
		case 645:
			sendNpcChat("Welcome to Tyras.", "You have the option to play as an <col=" + Right.IRONMAN.getColor() + "><img=12></img>Iron Man</col>,",
		...
		case 649:
			sendNpcChat("Goodluck on your adventure.");
		...
		case 712:
			sendNpcChat("To teleport through the world simply click the world-map", "icon on the top-right of the screen or", "click the following on your keyboard (CTRL+T)", "Or if you prefer a command, type ::teleport");
			c.nextChat = 710;
			break;

		case 713:
			sendNpcChat("Yes, you must follow rules of this world to the stone", "Make sure you are aware of them! You can", "find and read them by typing ::rules");
			c.nextChat = 710;
			break;
		...
		case 715:
			sendNpcChat("If you have been lucky enough to get your hands on", "one of those precious tickets, you can exchange", "them into points by using the button right", "inside the bank of edgeville!");
			c.nextChat = 710;
			break;
		}
