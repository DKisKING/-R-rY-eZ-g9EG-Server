package osv.model.players.skills.thieving;
...
/**
 * A representation of the thieving skill. Support for both object and npc action	s will be supported.
 * 
...
public class Thieving {
	...
	public static int[] rogueOutfit = { 5553, 5554, 5555, 5556, 5557 };
	...
	public void steal(Stall stall, int objectId, Location3D location) {
		...
		int pieces = 0;
		for (int i = 0; i < rogueOutfit.length; i++) {
			if (player.getItems().isWearingItem(rogueOutfit[i])) {
				pieces += 2;
			}
		}
		...
		if (!Server.getGlobalObjects().exists(objectId, location.getX(), location.getY()) || Server.getGlobalObjects().exists(634, location.getX(), location.getY())) {
	...
	public void steal(Pickpocket pickpocket, NPC npc) {
		...
		if (item != null) {
			player.getItems().addItem(item.getId(), maxCape && Pickpocket.FARMER != null ? item.getAmount() * 2 : item.getAmount());
		} else {
	...
	public enum Pickpocket {
		MAN(1, 8, 55000, new HashMap<Rarity, List<GameItem>>() {
		...
		}), FARMER(60, 65, 52000, new HashMap<Rarity, List<GameItem>>() {
			{
				...
				put(Rarity.COMMON, Arrays.asList(new GameItem(5294), new GameItem(5297)));
				put(Rarity.UNCOMMON, Arrays.asList(new GameItem(5295), new GameItem(5296), new GameItem(5298), new GameItem(5301), new GameItem(5302)));
				put(Rarity.RARE, Arrays.asList(new GameItem(5299), new GameItem(5300), new GameItem(5303)));
				put(Rarity.VERY_RARE, Arrays.asList(new GameItem(5304)));
			}
		 ...
		 */
		private Pickpocket(int level, int experience, int petChance, Map<Rarity, List<GameItem>> items) {
			this.level = level;
	...
	public enum Stall {
		CAKE(new GameItem(1893), 5, 16, 20, 45000),
		TEA(new GameItem(712), 5, 16, 10, 43000),
		FUR(new GameItem(6814), 35, 36, 15, 42000),
		SILVER(new GameItem(2961), 50, 54, 10, 40000),
		GEM(new GameItem(1623), 75, 80, 10, 38000);
		 ...
		 */
		private Stall(GameItem item, int level, int experience, int depletionProbability, int petChance) {
			this.item = item;
			this.level = level;
			this.experience = experience;
