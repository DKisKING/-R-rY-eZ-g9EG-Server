...
import osv.model.items.bank.BankTab;
...
public class ItemAssistant {
	...
	public boolean isWearingItem(int itemId, int slot) {
		if (slot < 0 || slot > c.playerEquipment.length - 1) {
			return false;
		}
	...
	public boolean isNoted(int itemId) {
		...
		if (list == null || list.itemDescription == null) {
			return false;
		}

		return list.itemDescription.startsWith("Swap this note at any bank");
	}
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
			...
			c.getPA().sendFrame126(WeaponName, 12293);
				...
				|| WeaponName.startsWith("seercull") || WeaponName.contains("blowpipe") || WeaponName.contains("ballista") || WeaponName.contains("thrownaxe")) {
		...
		} else if ((WeaponName.startsWith("Staff") || WeaponName.endsWith("staff") || WeaponName.endsWith("of the seas") || WeaponName.endsWith("wand"))
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		if (itemName.contains("godsword") || itemName.contains("crystal") || itemName.contains("aradomin sword")
				...
				itemName.contains("clueless") || itemName.contains("ballista") || itemName.contains("hunting knife") || itemName.contains("elder maul") || itemName.contains("bulwark")) {
			return true;
		}
		switch (itemId) {
		case 12926:
