...
public class ShopAssistant {
	...
	public void openShop(int ShopID) {
		...
		case 118:
			c.getPA().sendFrame126(ShopHandler.ShopName[ShopID] + " - " + c.raidPoints + " Raid Points", 64003);
			break;
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 118) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + getSpecialItemValue(removeId) + " raids points.");
			return;
		}
		if (c.myShopId == 12) {
			switch (removeId) {
		...
		if (c.myShopId == 77) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + (removeId != 1464 ? getSpecialItemValue(removeId) * 5 : getSpecialItemValue(removeId)) + " vote points.");
			return;
		}
		if (c.myShopId == 78) {
	...
	public int getSpecialItemValue(int id) {
		...
		case 2572:
			if (c.myShopId == 77)
				return c.getMode().isIronman() ? 30 : 10;
		...
		case 6199:
			if (c.myShopId == 77)
				return c.getMode().isIronman() ? 45 : 20;
			else if (c.myShopId == 78)
				return 30;
			else
				return 20;
			...
			for (int i = amount; i > 0; i--) {
				...
				if (c.getItems().freeSlots() > 0 || c.getItems().playerHasItem(995)) {
					if (Item.itemIsNote[itemID] == false) {
						c.getItems().deleteItemNoSave(itemID, c.getItems().getItemSlot(itemID), 1);
					} else {
						c.getItems().deleteItemNoSave(itemID, fromSlot, 1);
					}
					if (c.myShopId == 12) {
						c.getItems().addItem(2996, (int) Math.ceil(TotPrice3 *= 0.75));
					} else if (c.myShopId != 12 && c.myShopId != 29 && c.myShopId != 44 && c.myShopId != 18 && c.myShopId != 116 && c.myShopId != 115) {
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
        ...
        if (c.myShopId == 16) {
            if (c.getMode().isIronman() || c.getMode().isUltimateIronman()) {
                if (!c.getDiaryManager().getFaladorDiary().hasDoneMedium()) {
                    c.sendMessage("You must have completed the Falador diary up to medium to purchase this.");
                    return false;
                }
            }
        }
		...
		if (c.myShopId == 12) {
			...
			if (c.getSlayer().getPoints() >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					...
					c.getItems().addItem(itemID, amount);
					c.getItems().resetItems(3823);
					c.getPA().sendFrame126("@whi@Slayer Points: @gre@" + c.getSlayer().getPoints(), 7333);
				}
			} else {
			...
			if (c.raidPoints >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					...
					c.getPA().loadQuests();
					c.getPA().sendFrame126(ShopHandler.ShopName[118] + " - " + c.raidPoints + " Raid Points", 64003);
					c.getItems().addItem(itemID, amount);
					c.getItems().resetItems(3823);
				}
			} else {
				c.sendMessage("You do not have enough raid points to buy this item.");
			}
			...
			if (c.votePoints >= (itemID != 1464 ? getSpecialItemValue(itemID) * 5 * amount : getSpecialItemValue(itemID) * amount)) {
				if (c.getItems().freeSlots() > 0) {
					c.votePoints -= itemID != 1464 ? getSpecialItemValue(itemID) * 5 * amount : getSpecialItemValue(itemID) * amount;
					c.getPA().loadQuests();
					c.getItems().addItem(itemID, amount);
					c.getItems().resetItems(3823);
				}
